...
const DATASETS: Record<string, DatasetConfig> = {
	cutlass: {
		...
		r2Url: 'https://assets.harryzorus.xyz/datasets/cutlass-dataset.zip',
		maxAgeHours: 24 * 7,
		queries: {
	...
	pytorch: {
		...
		r2Url: 'https://assets.harryzorus.xyz/datasets/pytorch-dataset.zip',
		maxAgeHours: 24 * 7,
		queries: {
...
// =============================================================================
// STEP 1: IDEMPOTENT DATASET DOWNLOAD FROM R2
// =============================================================================

function shouldDownload(datasetDir: string, maxAgeHours: number): boolean {
...
async function downloadAndExtractDataset(config: DatasetConfig): Promise<boolean> {
	log(`Downloading from ${config.r2Url}...`, 'info');

	try {
		// Download zip file
		const response = await fetch(config.r2Url);
		if (!response.ok) {
			log(`Download failed: ${response.status} ${response.statusText}`, 'error');
			return false;
		}

		const zipData = new Uint8Array(await response.arrayBuffer());
		const zipPath = join(DATA_DIR, `${config.name}-dataset.zip`);
		...
		log(`Downloaded ${(zipData.length / 1024).toFixed(0)} KB`, 'success');
		...
		try {
			await Deno.remove(config.dir, { recursive: true });
		} catch {
			// Directory doesn't exist, that's fine
		}

		// Extract to temp location then move
		const extractDir = join(DATA_DIR, `${config.name}-dataset`);
		const unzipSuccess = await runCommand(['unzip', '-o', '-q', zipPath, '-d', DATA_DIR]);
		if (!unzipSuccess) {
		...
		await Deno.rename(extractDir, config.dir);
...
async function ensureDataset(config: DatasetConfig, forceDownload: boolean): Promise<boolean> {
	console.log(`\nDataset: ${config.displayName}`);

	if (!forceDownload && !shouldDownload(config.dir, config.maxAgeHours)) {
		log('Fresh (skipping download)', 'success');
		return true;
	}

	return await downloadAndExtractDataset(config);
}
...
async function compileSorex(): Promise<boolean> {
	...
	const binaryMtime = existsSync(binaryPath)
		...
		: 0;
	const wasmMtime = existsSync(wasmPath) ? (Deno.statSync(wasmPath).mtime?.getTime() ?? 0) : 0;
	...
	// WASM is embedded in .sorex files during index build, no separate WASM build needed
	log('WASM embedded in .sorex files', 'success');
...
function parseSorexOutput(output: string): SorexTierTiming | null {
	// Note: CLI output uses µs (Unicode micro sign U+00B5) not us
	const t1Match = output.match(
		/T1 Exact:\s*([\d.]+)\s*(µs|us|ms)\s*\((\d+)\s*(?:hits?|results?)\)/
	);
	const t2Match = output.match(
		/T2 Prefix:\s*([\d.]+)\s*(µs|us|ms)\s*\((\d+)\s*(?:hits?|results?)\)/
	);
	const t3Match = output.match(
		/T3 Fuzzy:\s*([\d.]+)\s*(µs|us|ms)\s*\((\d+)\s*(?:hits?|results?)\)/
	);
	const totalMatch = output.match(/Search total:\s*([\d.]+)\s*(µs|us|ms)/);
...
async function runSorexBenchmark(
	indexPath: string,
	query: string,
	config: BenchConfig
): Promise<SorexBenchResult> {
...
async function benchmarkDataset(
	...
	console.log(
		`   Adaptive: min=${benchConfig.minIterations}, max=${benchConfig.maxIterations}, target CI=${benchConfig.targetCIPercent}%`
	);
	...
	for (const tier of ['exact', 'prefix', 'fuzzy'] as const) {
		...
		for (const query of config.queries[tier]) {
			...
			const libraries = [
				{
					name: 'FlexSearch',
					search: () => flexIndex.search(query).flatMap((r) => r.result ?? [])
				},
			...
			for (const lib of libraries) {
				const { samples, resultCount, iterations, converged } = measureLibrarySearch(
					lib.search,
					benchConfig
				);
	...
	for (const tier of ['exact', 'prefix', 'fuzzy'] as const) {
		...
		for (const query of config.queries[tier]) {
			...
			for (const lib of queryLibs) {
				lines.push(
					`| ${query} | ${lib.library} | - | - | ${lib.meanUs.toFixed(1)} | ${lib.resultCount} |`
				);
			}
		}
		lines.push('');
...
async function main(): Promise<void> {
	const args = parseArgs(Deno.args, {
		string: ['dataset'],
		boolean: ['skip-download', 'skip-index', 'skip-compare', 'quick', 'force-download'],
		default: {
			'skip-download': false,
			'skip-index': false,
			'skip-compare': false,
	...
	console.log(
		`   Warmup: ${benchConfig.warmupIterations}, Measured: ${benchConfig.measuredIterations}`
	);
	console.log(`   Confidence: ${benchConfig.confidenceLevel * 100}%`);

	// Step 1: Ensure datasets (download from R2 if needed)
	if (!args['skip-download']) {
		for (const key of datasetKeys) {
			...
			await ensureDataset(config, args['force-download']);
		}
	}
	...
	if (!args['skip-compare']) {
		for (const key of datasetKeys) {
			...
			if (libraryResults.length > 0 && sorexResultsByDataset[key]) {
				const comparisonMd = generateLibraryComparisonMarkdown(
					key,
					sorexResultsByDataset[key],
					libraryResults
				);
