//! Type-level invariants for verified search operations.
//!
//! This module provides wrapper types that encode invariants at the type level,
//! making it impossible to create invalid states. This enables safer refactoring
//! by letting the compiler verify that invariants are preserved.
//!
//! # Lean Correspondence
//!
//! Each type here corresponds to a specification in the Lean project:
//! - `ValidDocId` → `SuffixEntry.WellFormed` (Types.lean)
//! - `ValidOffset` → `SuffixEntry.WellFormed` (Types.lean)
//! - `SortedSuffixArray` → `SuffixArray.Sorted` (SuffixArray.lean)
//! - `WellFormedIndex` → `SearchIndex.WellFormed` (Types.lean)
//!
//! # Usage
//!
//! ```ignore
//! // Create a validated index (checks all invariants at construction)
//! let index = WellFormedIndex::new(docs, texts, field_boundaries)?;
//!
//! // Type system now guarantees invariants
//! let results = index.search("query"); // Cannot panic on index bounds
//! ```
